...
def _add_rel_pos(
    attn: torch.Tensor,
    q: torch.Tensor,
    q_hw: Tuple[int, int, int],
    k_hw: Tuple[int, int, int],
    ...
    r_q = q[:, :, 1:].reshape(B, n_head, q_h, q_w, dim)
    rel_h_q = torch.einsum("byhwc,hkc->byhwk", r_q, Rh)  # [B, H, q_t, qh, qw, k_h]
    rel_w_q = torch.einsum("byhwc,wkc->byhwk", r_q, Rw)  # [B, H, q_t, qh, qw, k_w]
    # [B, H, q_t, q_h, q_w, dim] -> [q_t, B, H, q_h, q_w, dim] -> [q_t, B*H*q_h*q_w, dim]
    r_q = r_q.reshape(B * n_head * q_h * q_w, dim)

    # Combine rel pos.
    rel_pos = (
        rel_h_q[:, :, :, :, None, :, None]
        + rel_w_q[:, :, :, :, None, None, :]
    ).reshape(B, n_head, q_h * q_w, k_h * k_w)
...
class MultiscaleBlock(nn.Module):
    def __init__(
        ...
        if _prod(cnf.stride_q) > 1:
            ...
            self.pool_skip = Pool(
                nn.MaxPool2d(kernel_skip, stride=cnf.stride_q, padding=padding_skip), None  # type: ignore[arg-type]
            )
...
class PositionalEncoding(nn.Module):
    def __init__(self, embed_size: int, spatial_size: Tuple[int, int], rel_pos_embed: bool) :
        super().__init__()
